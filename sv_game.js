    ...
    constructor(owner) {
        ...
        db.get('SELECT * FROM critters WHERE owner = ?', [owner.username], (err, critter) => {
            if (err) {
                ...
                this.resetCritter();
                db.run('INSERT INTO critters (owner, alive, life, heartbeats, energy, GLUT, PRIDE, SLOTH, hunger, boredom, happiness, BODY, MIND, SOUL, breed, heartrate) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
                    [owner.username, this.alive, this.health.life, this.health.heartbeats, this.health.energy, this.vices.GLUT, this.vices.PRIDE, this.vices.SLOTH, this.stats.hunger, this.stats.boredom, this.stats.happiness, this.traits.BODY, this.traits.MIND, this.traits.SOUL, this.breed, this.heartrate], (err) => {
                        if (err) {
                            console.error(err.message);
                        }
                    });
...
class Game {
    ...
    step() {
        ...
        for (const critter of this.critters) {
            if (critter.alive) {
                ...
                db.run('UPDATE critters SET alive = ?, life = ?, heartbeats = ?, energy = ?, GLUT = ?, PRIDE = ?, SLOTH = ?, hunger = ?, boredom = ?, happiness = ?, BODY = ?, MIND = ?, SOUL = ?, breed = ?, heartrate = ? WHERE owner = ?',
                    [critter.alive, critter.health.life, critter.health.heartbeats, critter.health.energy, critter.vices.GLUT, critter.vices.PRIDE, critter.vices.SLOTH, critter.stats.hunger, critter.stats.boredom, critter.stats.happiness, critter.traits.BODY, critter.traits.MIND, critter.traits.SOUL, critter.breed, critter.heartrate, critter.owner.username],
                    (err) => {
                        if (err) {
                            console.error(err.message);
                        }
    ...
    evolve(critter) {
        switch (critter.health.heartbeats) {
            case 6:
                ...
                switch (rand) {
                    case 0:
                        critter.breed = 'goob';
                        critter.traits.MIND += 10;
                        break;
                    case 1:
                        this.breed = 'wibber';
                        critter.traits.SOUL += 10;
                        break;
                    case 2:
                        this.breed = 'xorby';
                        critter.traits.BODY += 10;
                        break;
                    default:
                        break;
                }
